...
function Top10(){
    ...
    const handleNextSlide = () => {
        if(sliderIndex <= 12 % 6){
            setSliderIndex((prevIndex) => (prevIndex + 1) % 12);
        }else{
            setSliderIndex(0);
        }
    };
    ...
    const handlePrevSlide = () => {
        if(sliderIndex > 0){
            setSliderIndex(
                (prevIndex) => (prevIndex - 1 + results.length) % 10
            );
        }
    };
    ...
    return (
        <div className="flex flex-col justify-center pt-6 gap-2 newRelease-container">
            <h1 className="newRelease-heading pl-12">Top 10 Movies</h1>
            <div className="newRelease-carousal">
                ...
                <div className="slider" style={{ transform: `translateX(calc(-${sliderIndex * 100}%))`, transition: "transform 0.5s ease-in-out", }}>
                    {results?.slice(0, 10)?.map((item, index) => (
                        <div key={item.id} className="top10-carousal-item flex content-center">
                            <div className="text-white">
                                {svgArr[index]?.svg}
                            </div>

                            <div className="top10-carousal-cards" style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original${item.poster_path})` }}></div>
